...
void* gpu_usm::lock(const stream& stream, mem_lock_type type) {
    ...
    if (0 == _lock_count) {
        ...
        if (alloc_type == allocation_type::usm_device || alloc_type == allocation_type::cl_mem) {
            ...
            OV_ZE_EXPECT(zeCommandListAppendMemoryCopy(_ze_stream.get_cp_queue(),
                                    ...
                                    0,
                                    nullptr));
            OV_ZE_EXPECT(ze::zeCommandListHostSynchronize(_ze_stream.get_cp_queue(), endless_wait));
    ...
    if (0 == _lock_count) {
        if (_copy_back_to_device) {
                ...
                OV_ZE_EXPECT(zeCommandListAppendMemoryCopy(_ze_stream.get_cp_queue(),
                                        ...
                                        0,
                                        nullptr));
                OV_ZE_EXPECT(zeCommandListHostSynchronize(_ze_stream.get_cp_queue(), endless_wait));
        }
...
event::ptr gpu_usm::fill(stream& stream, unsigned char pattern, const std::vector<event::ptr>& dep_events, bool blocking) {
    ...
    OV_ZE_EXPECT(ze::zeCommandListAppendMemoryFill(_ze_stream.get_cp_queue(),
        _buffer.handle().ptr,
        &pattern,
...
event::ptr gpu_usm::copy_from(stream& stream, const void* data_ptr, size_t src_offset, size_t dst_offset, size_t size, bool blocking) {
    ...
    OV_ZE_EXPECT(ze::zeCommandListAppendMemoryCopy(_ze_stream->get_cp_queue(),
                                           dst_ptr,
                                           src_ptr,
                                           size,
                                           _ze_event,
...
void* gpu_image2d::lock(const stream& stream, mem_lock_type type) {
    ...
    if (0 == _lock_count) {
        ...
        if (type != mem_lock_type::write) {
            OV_ZE_EXPECT(ze::zeCommandListAppendImageCopyToMemory(zero_stream.get_cp_queue(),
                _host_buffer.handle().ptr,
            ...
            OV_ZE_EXPECT(ze::zeCommandListHostSynchronize(zero_stream.get_cp_queue(), endless_wait));
        }
        _mapped_ptr = _host_buffer.handle().ptr;
    }
    _lock_count++;
...
void gpu_image2d::unlock(const stream& stream) {
    ...
    if (0 == _lock_count) {
        if (_needs_write_back) {
            auto& zero_stream = downcast<const ze_stream>(stream);
            OV_ZE_EXPECT(ze::zeCommandListAppendImageCopyFromMemory(zero_stream.get_cp_queue(),
                _image_holder.handle(),
                _host_buffer.handle().ptr,
                nullptr,
                nullptr,
            ...
            OV_ZE_EXPECT(ze::zeCommandListAppendBarrier(zero_stream.get_cp_queue(), nullptr, 0, nullptr));
        }
        _host_buffer.drop();
        _mapped_ptr = nullptr;
    }
...
event::ptr gpu_image2d::fill(stream& stream, unsigned char pattern, const std::vector<event::ptr>& dep_events, bool blocking) {
    ...
    OV_ZE_EXPECT(ze::zeCommandListAppendMemoryFill(zero_stream.get_cp_queue(),
...
event::ptr gpu_image2d::copy_from(stream& stream, const memory& src_mem, size_t src_offset, size_t dst_offset, size_t size, bool blocking) {
    ...
    auto ev_result_handle = downcast<ze::ze_base_event>(result_event.get())->get_handle();
    OV_ZE_EXPECT(ze::zeCommandListAppendImageCopy(zero_stream->get_cp_queue(),
        _image_holder.handle(),
