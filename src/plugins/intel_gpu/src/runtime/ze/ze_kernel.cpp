...
void ze_kernel::create_kernels_from_module(std::shared_ptr<ze_module_holder> module, std::vector<kernel::ptr> &out) {
    ...
    std::lock_guard lock(module->get_mutex());
...
std::shared_ptr<kernel> ze_kernel::clone(bool reuse_kernel_handle = false) const {
    if (reuse_kernel_handle) {
        ...
        {
            std::lock_guard lock(m_kernel->get_module()->get_mutex());
...
bool ze_kernel::is_same(const kernel &other) const {
...
void ze_kernel::set_arguments(const kernel_arguments_desc& args_desc, const kernel_arguments_data& args) {
    ...
    std::lock_guard lock(m_kernel->get_mutex());
...
void ze_kernel::launch(ze_command_list_handle_t cmd_list_handle, const kernel_arguments_desc& args_desc, const kernel_arguments_data& args, const std::vector<event::ptr>& deps, event::ptr out_event) {
    ...
    auto wait_event_ptr = wait_event_handles.size() == 0 ? nullptr : wait_event_handles.data();
    auto out_event_ptr = ze_out_event == nullptr ? nullptr : ze_out_event->get_handle();
    ze_group_count_t groups = { global.groupCountX / local.groupCountX, global.groupCountY / local.groupCountY, global.groupCountZ / local.groupCountZ };

    // Assume single threaded execution - lock is not needed
    OV_ZE_EXPECT(zeKernelSetGroupSize(kernel_handle, local.groupCountX, local.groupCountY, local.groupCountZ));
    OV_ZE_EXPECT(zeCommandListAppendLaunchKernel(cmd_list_handle,
                                             kernel_handle,
                                             &groups,
                                             out_event_ptr,
                                             wait_event_handles.size(),
                                             wait_event_ptr));
}
