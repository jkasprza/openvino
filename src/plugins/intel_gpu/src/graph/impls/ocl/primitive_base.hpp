...
protected:
    ...
    event::ptr add_to_cmd_list_impl(const std::vector<event::ptr>& events,
                                    typed_primitive_inst<PType>& instance) override {
        stream& stream = instance.get_network().get_stream();
        command_list::ptr cmd_list = instance.get_cmd_list();
        if (instance.can_be_optimized()) {
            return stream.aggregate_events(events, events.size() > 1, instance.is_output());
        }
        std::vector<event::ptr> tmp_events(events);
        ...
        for (size_t kd_idx = 0; kd_idx < _kernel_data.kernels.size(); ++kd_idx) {
            ...
            GPU_DEBUG_TRACE_DETAIL << "A kernel " << kd_idx << ": gws=[" << gws[0] << ", " << gws[1] << ", " << gws[2] << "] "
                                   << "lws=[" << lws[0] << ", " << lws[1] << ", " << lws[2] << "]"
                                   << (needs_completion_event ? " has_completion_event=true" : "") << std::endl;

            auto ev = cmd_list->append_kernel_launch(*_kernels[kd_idx], params, args, tmp_events, needs_completion_event);
            if (_kernel_data.needs_sub_kernels_sync) {
                tmp_events = {ev};
            }
            all_events.push_back(ev);
