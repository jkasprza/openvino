...
#include "gpu_spirv_kernel_sources.inc"
    };
    ...
    OPENVINO_THROW("SPIRV Kernel template ", template_name, " not found");
}

std::string_view SourcesDB::get_kernel_header(std::string_view header_name) {
    static std::array headers = {
#include "gpu_spirv_kernel_headers.inc"
    };
    ...
    OPENVINO_THROW("SPIRV Kernel header ", header_name, " not found");
}
